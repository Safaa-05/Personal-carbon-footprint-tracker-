...
from .anonymous_calculator import (
    calculate_electricity, calculate_cooking,
    calculate_transport, calculate_food, calculate_waste
)
...
# ---------------- Guest / Anonymous ----------------
def calculate_guest(data: dict) -> dict:
    return {
...
def calculate_logged(data: dict) -> dict:
    try:
        electricity = data["electricity_kwh"] * (
            FACTORS["energy"]["renewable"] if data["renewable"] else FACTORS["energy"]["electricity"]
        )
        ...
        waste = data.get("food_waste_kg", 0) * FACTORS["food"]["waste"]
        food = baseline + red_meat + waste
